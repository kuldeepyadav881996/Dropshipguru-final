...
| `GOOGLE_SCRIPT_URL` | Backend `/api/payment/verify` | Config | Google Apps Script Web App URL for paid leads |
...
GOOGLE_SCRIPT_URL=https://script.google.com/macros/s/AKfycbzpPiH8gpzHvk9-AEHEKDq5WT7XnEsfJVIIz-ki3-BCprd4xLGaNQ2FJFHoyfy-qtlafQ/exec
...
Browser                     Backend (.env secrets)              Razorpay
   ...
   |   (+ full customer fields)     |-- POST form data ----------->| Google Apps Script → Sheet
   |<- { verified: true }  or 401 --| (Sheets failure does not fail payment)
```
